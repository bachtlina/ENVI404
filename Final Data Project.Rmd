...
##Introduction
     Data was collected using an acoustic Doppler velocimeter and a dissolved oxygen logger. For each site, one along the Allegheny River and two along the Monongahela River, graphs of height vs dissolved oxygen and height vs velocity are shown. The Reynolds Stress Tensors are also shown for four different depths.
...
Normal stresses are diagonal in the tensor and sheer stresses are off-diagonal. The turbulent fluctuations exert a stress on the flow. If the graph appears isotrophic, there is no directional preference but if the graph appears anisotropic 
...
p1 <- ggplot(DO528AR214, aes(x = DO.mg.l. , y= height.m.))+
  ...
  theme(axis.text = element_text(face = "plain", size = 6)) +
  theme(aspect.ratio = 24/9)
...
p3 <- ggplot(df528AR214, aes(x= wi, y= vi)) +
  ...
  theme(axis.text = element_text(face = "plain", size = 4))
...
```{r warning=FALSE, echo=FALSE}
grid.arrange(p13, p14, arrangeGrob(p15, p16, p17, p18, nrow=4), nrow=1, top = "Monongahela River (Birmingham Bridge")
```